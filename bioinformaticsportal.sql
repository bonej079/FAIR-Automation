...
(11, 8, 5, 5, 0, 12, 'PhosphoPICK is a method for predicting kinase substrates using an integrated system of cellular context and protein sequence information, and is currently able to make predictions for 107 human kinases, 24 mouse kinases and 26 yeast kinases.', 'https://doi.org/10.1093/bioinformatics/btx072\r\n', 'http://bioinf.scmb.uq.edu.au/phosphopick/download'),
(12, 8, 5, 5, 2, 13, 'Breakpointer is a fast tool for locating sequence breakpoints from the alignment of single end reads (SE) produced by next generation sequencing (NGS). It adopts a heuristic method in searching for local mapping signatures created by insertion/deletions (indels) or more complex structural variants(SVs). With current NGS single-end sequencing data, the output regions by Breakpoint mainly contain the approximate breakpoints of indels and a limited number of large SVs. Notably, Breakpointer can uncover breakpoints of insertions which are longer than the read length. Breakpointer also can find breakpoints of many variants located in repetitive regions. The regions can be used not only as a extra support for SV predictions by other tools (such as by split-read method), but also can serve as a database for searching variants which might be missed by other tools. Breakpointer is a command line tool that runs under linux system.', 'https://doi.org/10.1093/bioinformatics/bts064\r\n', 'https://github.com/ruping/Breakpointer'),
(13, 8, 5, 5, 2, 14, 'HMMER is used for searching sequence databases for sequence homologs, and for making sequence alignments. It implements methods using probabilistic models called profile hidden Markov models (profile HMMs). ', 'https://doi.org/10.1093/nar/gkr367\r\n', 'http://hmmer.org/download.html'),
...
(15, 8, 5, 5, 2, 16, 'InterPro is a database which integrates together predictive information about proteins\' function from a number of partner resources, giving an overview of the families that a protein belongs to and the domains and sites it contains.\r\n\r\nUsers who have novel nucleotide or protein sequences that they wish to functionally characterise can use the software package InterProScan to run the scanning algorithms from the InterPro database in an integrated way. Sequences are submitted in FASTA format. Matches are then calculated against all of the required member database\'s signatures and the results are then output in a variety of formats.', 'https://doi.org/10.1093/bioinformatics/btu031\r\n', 'https://github.com/ebi-pf-team/interproscan'),
(16, 8, 5, 5, 2, 17, 'MUSCLE is one of the best-performing multiple alignment programs according to published benchmark tests, with accuracy and speed that are consistently better than CLUSTALW. MUSCLE can align hundreds of sequences in seconds. Most users learn everything they need to know about MUSCLE in a few minutes—only a handful of command-line options are needed to perform common alignment tasks.', '10.1093/nar/gkh340\r\n', 'https://www.drive5.com/muscle/downloads.htm'),
